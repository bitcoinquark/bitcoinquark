...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-bootstrap", _("Enable BitcoinQuark bootstrap mode. Allows BTQ client to connect to Bitcoin p2p network to download blockchain history."));
    ...
    if (showDebug) {
        ...
        strUsage += HelpMessageOpt("-syncatheight", strprintf("Stop synchronizing blocks after reaching the given height in the main chain (default: %u)", DEFAULT_SYNCATHEIGHT));
...
bool AppInitParameterInteraction(Config& config)
{
    ...
    if (nProposedExcessiveBlockSize < (int)ONE_MEGABYTE) {
        return InitError(
...
bool AppInitMain(const Config& config)
{
    const CChainParams& chainparams = Params();
    ...
    fBTQBootstrapping = gArgs.GetBoolArg("-bootstrap", false);
