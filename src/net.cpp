    ...
    for (const CDNSSeedData &seed : vSeeds) {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, 0, true))
            {
                ...
                {
                    int nOneDay = 24*3600;
                    CAddress addr = CAddress(CService(ip, Params().GetDefaultPort(fBTQBootstrapping)), requiredServiceBits);
    ...
    while (!interruptNet)
    {
        ...
        while (!interruptNet)
        {
            ...
            if ((addr.GetPort() != Params().GetDefaultPort() ||
            		(fBTQBootstrapping && addr.GetPort() != Params().GetDefaultPort(true)))
            		&& nTries < 50)
                continue;
...
void CConnman::ThreadMessageHandler()
{
    while (!flagInterruptMsgProc)
    {
        ...
        for (CNode* pnode : vNodesCopy)
        {
            ...
            // Receive messages
            bool fMoreNodeWork = m_msgproc->ProcessMessages(GetConfig(), pnode, flagInterruptMsgProc);
            ...
            // Send messages
            {
                ...
                m_msgproc->SendMessages(GetConfig(), pnode, flagInterruptMsgProc);
            }
...
bool CConnman::OutboundTargetReached(bool historicalBlockServingLimit)
{
    ...
    if (historicalBlockServingLimit)
    {
        ...
        uint64_t buffer = timeLeftInCycle / 600 * nMaxBlockWeight;
