...
double GetDifficultyBitcoinQuark(const CChain& chain, const CBlockIndex* blockindex)
{
 ...
 */
double GetDifficultyBitcoin(const CChain& chain, const CBlockIndex* blockindex)
{
...
double GetDifficulty(const CChain& chain, const CBlockIndex* blockindex)
{
    ...
    if(blockindex->nHeight >= Params().GetConsensus().BTQHeight)
    {
    	return GetDifficultyBitcoinQuark(chainActive, blockindex);
    }
    else
    {
    	return GetDifficultyBitcoin(chainActive, blockindex);
    }
}
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    int legacy_block_flag = (blockindex->nHeight < consensusParams.BTQHeight) ? SERIALIZE_BLOCK_LEGACY : 0;
    result.pushKV("strippedsize", (int)::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | legacy_block_flag | SERIALIZE_TRANSACTION_NO_WITNESS));
    result.pushKV("size", (int)::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | legacy_block_flag));
...
UniValue getblockheader(const JSONRPCRequest& request)
{
    ...
    if (!fVerbose)
    {
    	int legacy_block_flag = legacy_format ? SERIALIZE_BLOCK_LEGACY : 0;
        CDataStream ssBlock(SER_NETWORK, PROTOCOL_VERSION | legacy_block_flag);
...
UniValue getblock(const JSONRPCRequest& request)
{
    ...
    {
    	int legacy_block_flag = legacy_format ? SERIALIZE_BLOCK_LEGACY : 0;
        CDataStream ssBlock(SER_NETWORK, PROTOCOL_VERSION | legacy_block_flag | RPCSerializationFlags());
        ssBlock << block;
        std::string strHex = HexStr(ssBlock.begin(), ssBlock.end());
        return strHex;
    }
...
UniValue verifychain(const JSONRPCRequest& request)
{
    ...
    return CVerifyDB().VerifyDB(GetConfig(), Params(), pcoinsTip.get(), nCheckLevel, nCheckDepth);
}
