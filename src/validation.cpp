...
public:
    ...
    bool ActivateBestChain(const Config& config, CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock);

    bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex);
    bool AcceptBlock(const Config& config, const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock);

    // Block (dis)connection on a given view:
    DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view);
    bool ConnectBlock(const Config& config, const CBlock& block, CValidationState& state, CBlockIndex* pindex,
...
private:
    bool ActivateBestChainStep(const Config& config, CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace);
    bool ConnectTip(const Config& config, CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool);
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // MAX_BLOCK_SIGOPS_PER_MB; we still consider this an invalid rather than
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.BTQHeight;
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    {
    	return PREMINE_COIN / consensusParams.BTQPremineWindow;
    }
...
bool CChainState::ConnectBlock(const Config& config, const CBlock& block, CValidationState& state, CBlockIndex* pindex,
    ...
    if (!CheckBlock(config, block, state, chainparams.GetConsensus(), !fJustCheck, !fJustCheck))
    ...
    CDiskTxPos pos(pindex->GetBlockPos(), GetSizeOfCompactSize(block.vtx.size()));
    std::vector<std::pair<uint256, CDiskTxPos> > vPos;
    vPos.reserve(block.vtx.size());    blockundo.vtxundo.reserve(block.vtx.size() - 1);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if ((uint64_t)nSigOpsCost > nMaxSigOpsCount * WITNESS_SCALE_FACTOR)
        {
 ...
 */
bool CChainState::ConnectTip(const Config& config, CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool){
    ...
    {
        CCoinsViewCache view(pcoinsTip.get());
        bool rv = ConnectBlock(config, blockConnecting, state, pindexNew, view, chainparams);
 ...
 */
bool CChainState::ActivateBestChainStep(const Config& config, CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace){
    AssertLockHeld(cs_main);
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        for (CBlockIndex *pindexConnect : reverse_iterate(vpindexToConnect)) {
            if (!ConnectTip(config, state, chainparams, pindexConnect, pindexConnect == pindexMostWork ? pblock : std::shared_ptr<const CBlock>(), connectTrace, disconnectpool)) {
                if (state.IsInvalid()) {
 ...
 */
bool CChainState::ActivateBestChain(const Config& config, CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {    // Note that while we're often called here from ProcessNewBlock, this is
    ...
    do {
        ...
        bool fInitialDownload;
        {
            ...
            if (!ActivateBestChainStep(config, state, chainparams, pindexMostWork, pblock && pblock->GetHash() == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace))
                return false;
...
bool ActivateBestChain(const Config& config, CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {
    return g_chainstate.ActivateBestChain(config, state, chainparams, std::move(pblock));
}

bool CChainState::PreciousBlock(const Config& config, CValidationState& state, const CChainParams& params, CBlockIndex *pindex){
    {
        LOCK(cs_main);
    ...
    return ActivateBestChain(config, state, params, std::shared_ptr<const CBlock>());
}

bool PreciousBlock(const Config& config, CValidationState& state, const CChainParams& params, CBlockIndex *pindex) {
    return g_chainstate.PreciousBlock(config, state, params, pindex);
}
...
bool CheckBlock(const Config& config, const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Size limits
    auto nMaxBlockWeight = config.GetMaxBlockWeight();

    if(block.vtx.size() * WITNESS_SCALE_FACTOR > nMaxBlockWeight)
    {
    ...
    if (currentBlockSize * WITNESS_SCALE_FACTOR > nMaxBlockWeight) {
        return state.DoS(100, false, REJECT_INVALID, "bad-blk-length", false,
                         "size limits failed");
    }
    ...
    auto nMaxSigOpsCount = GetMaxBlockSigOpsCount(currentBlockSize * WITNESS_SCALE_FACTOR);
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after BTQ fork.
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BTQHeight &&
        ...
        if(index < 0 && index >= (int)(block.vtx[0]->vout.size())) {
            return state.DoS(
...
bool CChainState::AcceptBlock(const Config& config, const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
...
bool ProcessNewBlock(const Config& config, const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
...

    {
        ...
        if (ret) {
            // Store to disk
            ret = g_chainstate.AcceptBlock(config, pblock, state, chainparams, &pindex, fForceProcessing, nullptr, fNewBlock);        }
        if (!ret) {
    ...
    if (!g_chainstate.ActivateBestChain(config, state, chainparams, pblock))        return error("%s: ActivateBestChain failed", __func__);

    return true;
}

bool TestBlockValidity(const Config& config, CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    AssertLockHeld(cs_main);
    ...
    if (!CheckBlock(config, block, state, chainparams.GetConsensus(), fCheckPOW, fCheckMerkleRoot))
        return error("%s: Consensus::CheckBlock: %s", __func__, FormatStateMessage(state));
    if (!ContextualCheckBlock(block, state, chainparams.GetConsensus(), pindexPrev))
        return error("%s: Consensus::ContextualCheckBlock: %s", __func__, FormatStateMessage(state));
    if (!g_chainstate.ConnectBlock(config, block, state, &indexDummy, viewNew, chainparams, true))        return false;
...
bool CVerifyDB::VerifyDB(const Config& config, const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    LOCK(cs_main);
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 1 && !CheckBlock(config, block, state, chainparams.GetConsensus()))
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            if (!ReadBlockFromDisk(block, pindex, chainparams.GetConsensus()))
                return error("VerifyDB(): *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
            if (!g_chainstate.ConnectBlock(config, block, state, pindex, coins, chainparams))                return error("VerifyDB(): *** found unconnectable block at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
        }
    }
...
bool CChainState::LoadGenesisBlock(const CChainParams& chainparams)
{
    ...
    try {
        ...
        if (blockPos.IsNull())            return error("%s: writing genesis block to disk failed", __func__);
...
bool LoadExternalBlockFile(const Config& config, const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp){
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    LOCK(cs_main);
                    CValidationState state;
                    if (g_chainstate.AcceptBlock(config, pblock, state, chainparams, nullptr, true, dbp, nullptr))                        nLoaded++;
                    if (state.IsError())
                        break;
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, chainparams.GetConsensus()))
                        {
                            ...
                            if (g_chainstate.AcceptBlock(config, pblockrecursive, dummy, chainparams, nullptr, true, &it->second, nullptr))                            {
                                nLoaded++;
