...
bool CWallet::SignTransaction(CMutableTransaction &tx)
{
    ...
    for (const auto& input : tx.vin) {
        ...
        SignatureData sigdata;
        if (!ProduceSignature(*this, TransactionSignatureCreator(&txNewConst, nIn, amount, SIGHASH_ALL | SIGHASH_FORKID), scriptPubKey, sigdata)) {
            return false;
        }
    ...
    int nBytes;
    {
        ...
        if (sign)
        {
            ...
            for (const auto& coin : selected_coins)
            {
                ...
                if (!ProduceSignature(*this, TransactionSignatureCreator(&txNewConst, nIn, coin.txout.nValue, SIGHASH_ALL | SIGHASH_FORKID), scriptPubKey, sigdata))
                {
                    strFailReason = _("Signing transaction failed");
                    return false;
                } else {
