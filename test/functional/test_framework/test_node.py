...
class TestNode():
    ...
    def __init__(self, i, datadir, rpchost, timewait, binary, stderr, mocktime, coverage_dir, extra_conf=None, extra_args=None, use_cli=False):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("BITCOIND", "bitcoinquarkd")
        else:
            self.binary = binary
        self.stderr = stderr
        ...
        self.cli = TestNodeCLI(os.getenv("BITCOINCLI", "bitcoinquark-cli"), self.datadir)
    ...
    def start(self, extra_args=None, stderr=None, *args, **kwargs):
        ...
        self.running = True
        self.log.debug("bitcoinquarkd started, waiting for RPC to come up")

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the bitcoinquarkd process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError('bitcoinquarkd exited with status {} during initialization'.format(self.process.returncode))
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. bitcoinquarkd still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        raise AssertionError("Unable to connect to bitcoinquarkd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, partial_match=False, *args, **kwargs):
        ...
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                self.log.debug('bitcoinquarkd failed to start: %s', e)
                self.running = False
                self.process = None
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes bitcoinquarkd to shutdown, so this method takes
...
class TestNodeCLI():
    """Interface to bitcoinquarkd-cli for an individual node"""
    ...
    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with bitcoinquarkd-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run bitcoinquarkd-cli command. Deserializes returned string as python object."""

        pos_args = [str(arg) for arg in args]
        named_args = [str(key) + "=" + str(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same bitcoinquarkd-cli call"
        ...
        self.log.debug("Running bitcoinquarkd-cli command: %s" % command)
